            ...
            <div class="p-3 mt-5 text-white rounded shadow-sm col-6 offset-3 card bg-info">
                <form action="{{ route('teacher.update', $teacher->id) }}" method="POST">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="positionID" class="form-control @error('positionID') is-invalid @enderror">
                                ...
                                @foreach ($positions as $item)
                                    <option value="{{ $item->id }}" @if (old('positionID', $teacher->position_id) == $item->id) selected @endif>
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                            @error('positionID')
                        ...
                        <div class="mb-3">
                            ...
                            <select name="departmentID" class="form-control @error('departmentID') is-invalid @enderror">
                                ...
                                @foreach ($departments as $item)
                                    <option value="{{ $item->id }}" @if (old('departmentID', $teacher->department_id) == $item->id) selected @endif>
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                            @error('departmentID')
                        ...
                        <div class="mb-3">
                            <input type="submit" value="Update Teacher" class="rounded shadow-sm btn btn-danger w-100">
                        </div>
                        <div class="mb-3">
                            <a href="{{ route('teacher.list') }}" class="rounded shadow-sm btn btn-success w-100">View
                                Teacher List</a>
                        </div>
                    </div>
                </form>
