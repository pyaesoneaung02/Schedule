            ...
            <div class="p-3 mt-5 text-white rounded shadow-sm col-6 offset-3 card bg-info">
                <form action="{{ route('room.update', $room->id)}}" method="POST">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="yearID" class="form-control @error('yearID') is-invalid @enderror">
                                ...
                                @foreach ($years as $item)
                                    <option value="{{ $item->id }}" @if (old('yearID', $room->year_id) == $item->id) selected @endif>{{ $item->name }}</option>
                                @endforeach
                            </select>
                            @error('yearID')
                                <span class="invalid-feedback">{{ $message }}</span>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="majorID" class="form-control @error('majorID') is-invalid @enderror">
                                ...
                                @foreach ($majors as $item)
                                    <option value="{{ $item->id }}" @if (old('majorID', $room->major_id) == $item->id) selected @endif>{{ $item->name }}</option>
                                @endforeach
                            </select>
                            @error('majorID')
                                <span class="invalid-feedback">{{ $message }}</span>
                        ...
                        <div class="mb-3">
                            <input type="submit" value="Update Room" class="rounded shadow-sm btn btn-danger w-100">
                        </div>
                        <div class="mb-3">
