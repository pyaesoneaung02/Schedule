            ...
            <div class="p-3 mt-5 text-white rounded shadow-sm col-6 offset-3 card bg-info">
                <form action="{{ route('major.update', $major->id)}}" method="POST">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="yearID" class="form-control @error('yearID') is-invalid @enderror">
                                ...
                                @foreach ($years as $item)
                                    <option value="{{ $item->id }}" @if (old('yearID', $major->year_id) == $item->id) selected @endif>
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                            @error('yearID')
                        ...
                        <div class="mb-3">
                            <input type="submit" value="Update Major" class="rounded shadow-sm btn btn-danger w-100">
                        </div>
                        <div class="mb-3">
