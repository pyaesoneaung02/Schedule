...
@section('content')
    <div class="container">
        <div class="row">
            <div class="p-3 text-white rounded shadow-sm col-6 offset-3 card bg-info mt-5">
                <form action="{{ route('subject.createSubject') }}" method="POST">
                    ...
                    <div class="card-body">
                                    ...
                                    <select name="yearID" class="form-control @error('yearID') is-invalid @enderror">
                                        ...
                                        @foreach ($years as $item)
                                            <option value="{{ $item->id }}"
                                                @if (old('yearID') == $item->id) selected @endif>{{ $item->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('yearID')
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="majorID" class="form-control  @error('majorID') is-invalid @enderror">
                                        ...
                                        @foreach ($majors as $item)
                                            <option value="{{ $item->id }}"
                                                @if (old('majorID') == $item->id) selected @endif>{{ $item->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('majorID')
                        ...
                        <div class="mb-3">
                            <input type="submit" value="Create Subject" class="rounded shadow-sm btn btn-danger w-100">
                        </div>
                        <div class="mb-3">
                            <a href="{{ route('subject.list') }}" class="rounded shadow-sm btn btn-success w-100">View
                                Subject List</a>
                        </div>
                    </div>
                </form>
