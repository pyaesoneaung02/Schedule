...
@section('content')
    <div class="container-fluid">
        ...
        <div class="mb-4 d-flex justify-content-between align-items-center">
            <div>
                <h2 class="font-weight-bold" style="color: #000 !important;">
                    <i class="mr-2 fa-solid fa-book text-primary"></i>
                    Subject List
                </h2>
        ...
        <div class="mb-4 border-0 shadow-sm card">
            <div class="card-body">
                ...
                <div class="d-flex flex-wrap gap-2">
                    ...
                    <!-- All Button -->
                    <a href="{{ route('subject.list', array_merge(request()->except('year', 'page'), ['year' => ''])) }}" 
                    ...
                    <!-- Year Buttons -->
                    @foreach ($years as $yr)
                        <a href="{{ route('subject.list', array_merge(request()->except('year', 'page'), ['year' => $yr->id])) }}" 
        ...
        @php
            $groupedSubjects = $subjects->getCollection()->groupBy('year_name');
        @endphp

        @forelse ($groupedSubjects as $yearName => $yearSubjects)
            <div class="mb-4 border-0 shadow-sm card">
                <div class="text-white card-header bg-primary d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge badge-light text-primary font-weight-bold" style="font-size: 0.9rem;">
                        {{ $yearSubjects->first()->total_year_subjects ?? $yearSubjects->count() }} Subjects
                    </span>
                </div>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table align-middle table-hover table-bordered">
                            ...
                            <tbody>
                                @foreach ($yearSubjects as $index => $item)
                                    <tr class="font-weight-bold" style="color: #000 !important;">
                                        ...
                                        <td style="color: #000 !important;">{{ $item->long_name }}</td>
                                        <td class="text-center" style="color: #000 !important;">{{ $item->short_name }}</td>
                                        <td class="text-center" style="color: #000 !important;">{{ $item->time_number }}</td>
                                        <td style="color: #000 !important;">{{ $item->academic_year_name }}</td>
                                        <td style="color: #000 !important;">{{ $item->major_name }}</td>
                                        <td style="color: #000 !important;">{{ $item->semester_name }}</td>
                                        <td class="text-center" style="color: #000 !important;">{{ $item->created_at->format('d M Y') }}</td>
                                        <td class="text-center">
        ...
        <!-- Back Button -->
        <div class="mt-4">
            <a href="{{ route('subject.create') }}" class="btn btn-outline-primary font-weight-bold">
